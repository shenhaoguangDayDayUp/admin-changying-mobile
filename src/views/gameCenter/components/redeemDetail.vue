   ...
   methods:{
        async getList(params = Object.assign({}, this.tableParams, this.queryParams)) {
            if(params.start&&params.end){
                try {
                    const { data} = await integralApi.expenseDetail(Object.assign({},{id:this.$route.params.code},{start:this.$date(params.start, "start"),end:this.$date(params.end, "end"),index:params.index}), {
                        headers: {
        ...
        async getTotal(params) {
            ...
            try {
                const {data } = await integralApi.expenseTotal(query, {
