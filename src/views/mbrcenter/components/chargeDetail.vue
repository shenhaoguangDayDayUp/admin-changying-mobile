    ...
    <div>
       ...
       <v-layout wrap row>
            <v-flex xs6 sm5 md3 offset-sm1 offset-lg1 class="btn-layout" >
                <v-btn  class='text-xs-right mx-0 px-0' flat color="primary">合计:{{amount}}</v-btn>  
            </v-flex>
        ...
        <k-table @pageChage='handleCurrentChange' :tableSource='list' :pageCofig='pageCofig' :page.sync='page'>
            <template slot-scope='props' slot='items'> 
                <tr>
                    ...
                    <td> {{ props.item.method|payMethod}}</td>
...
import {rechargeApi} from '@/api/api';
       ...
       return{
           list:{
                ...
                headers: [
                    ...
                    { text: '方式', sortable: false,  align: 'center' },
                    { text: '金额(￥)', sortable: false,   align: 'center', },
                ],
                items:[]
            },
            pageCofig:{
   ...
   methods:{
        async getList(params = Object.assign({}, this.tableParams, this.queryParams)) {
            if(params.start&&params.end){
                try {
                    const { data} = await rechargeApi.rechargeList(Object.assign({},{id:this.$route.params.code},{start:this.$date(params.start, "start"),end:this.$date(params.end, "end"),index:params.index}), {
                        headers: {
        ...
        async getTotal(params) {
            ...
            const query = Object.assign({},{start:this.$date(params.start, "start"),end:this.$date(params.end, "end"),index:params.index},{id:this.$route.params.code})
            try {
                const {data } = await rechargeApi.rechargeTotal(query, {
