      ...
      <v-flex xs5 offset-xs2
        ...
        <v-menu ref="end"
          ...
          <v-text-field slot="activator"
                        ...
                        label="结束时间"
                        readonly></v-text-field>
          <v-date-picker :max='max' :min='startMin'
                         locale='zh-Hans'
                         v-model="queryParams.end"
                         no-title
                         scrollable>
        ...
        <k-table @pageChage='handleCurrentChange'
          ...
          <template slot-scope='props'
            ...
            <tr @click='gotoRow(props)'>
              ...
              <td class="text-xs-center px-0">{{ props.item.type|prizeTypeFilter }}</td>
              <td style='color:blue'
                  @click.stop='gotoItem(props.item )'
                  class="text-xs-center">{{ props.item.type == "BET"?props.item.sender.code:props.item.receiver.code }}</td>
