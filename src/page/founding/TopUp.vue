          ...
          <template slot-scope='props'
            ...
            <tr @click='gotoRow(props)'>
              ...
              <td class="text-xs-center px-0"
                  style='width:300px'>{{ props.item.method|payMethod }}</td>
              <td style='color:blue'
                  @click.stop='gotoItem(props.item.receiver.code )'
                  class="text-xs-center">{{ props.item.receiver.code }}</td>
...
export default {
  ...
  methods: {
    async getTotal(params, token) {
      ...
      const { data } = await rechargesTotalApi.query(query, token);
      this.amount = data;
    },
    gotoRow(row) {
      // console.log(row)
